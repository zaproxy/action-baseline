...
async function run() {
    ...
    try {
        ...
        await processReport(token, workspace, plugins, currentRunnerID, issueTitle);
    } catch (error) {
...
async function processReport(token, workSpace, plugins, currentRunnerID, issueTitle) {
    ...
    let issues = await octokit.search.issuesAndPullRequests(
        new URLSearchParams()
        .append('q', `is:issue state:open repo:${owner}/${repo} ${issueTitle}`)
        .append('sort', 'updated')
        .toString()
    );
