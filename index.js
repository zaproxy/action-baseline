  ...
  try {
    ...
    if (
      !(
        ...
        String(failAction).toLowerCase() === "false"
      )
    ) {
      console.log(
        "[WARNING]: 'fail_action' action input should be either 'true' or 'false'"
      );
    }
    ...
    if (rulesFileLocation) {
      plugins = await common.helper.processLineByLine(
        `${workspace}/${rulesFileLocation}`
      );
    }

    // Create the files so we can change the perms and allow the docker non root user to update them
    await exec.exec(
      `touch ${jsonReportName} ${mdReportName} ${htmlReportName}`
    );
    await exec.exec(
      `chmod a+w ${jsonReportName} ${mdReportName} ${htmlReportName}`
    );

    await exec.exec(`docker pull ${docker_name} -q`);
    let command =
      `docker run -v ${workspace}:/zap/wrk/:rw --network="host" ` +
    ...
    try {
      ...
      if (
        (err.toString().includes("exit code 2") ||
          err.toString().includes("exit code 1")) &&
        String(failAction).toLowerCase() === "true"
      ) {
        console.log(
          `[info] By default ZAP Docker container will fail if it identifies any alerts during the scan!`
        );
        core.setFailed(
          "Scan action failed as ZAP has identified alerts, starting to analyze the results. " +
            err.toString()
        );
      } else {
        console.log(
